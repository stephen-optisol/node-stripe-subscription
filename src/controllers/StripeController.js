...
const StripeController = {
  ...
  async editPlan(req, res, next) {
    try {
      ...
      if (
        existingPlan.amount !== req.body.plan.amount * 100 ||
        existingPlan.interval !== req.body.plan.interval ||
        existingPlan.currency !== req.body.plan.currency
      ) {
  ...
  async upgradeSubscription(req, res, next) {
    try {
      ...
      if (revenue && revenue * 100 > plan.amount && stripeSubsciption) {
        ...
        const error = new ApiError(
          "Can't upgrade subscription plan.Annual Revenue is low",
          status.BAD_REQUEST,
        );
        next(error);
      }
